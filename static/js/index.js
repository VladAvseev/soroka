...
const priceListModal = new HTMLModalWindow({
    title: 'Цены',
    content: `
        <div class="price">
            <button class="price-btn" id="price-prev"><img class="about-btn-img" src="img/elements/left-arrow.png" alt=""></button>
            <div class="price-wrapper"></div>
            <button class="price-btn" id="price-next"><img class="about-btn-img" src="img/elements/right-arrow.png" alt=""></button>
        </div>
    `,
    footerButtons: [
...
function render() {
    ...
    createWorkSliders();
    createPriceSlider();
    fetch('https://soroka-31e2e-default-rtdb.europe-west1.firebasedatabase.app/promotion.json')
        .then(response => response.json())
        .then(res => {
            console.log(res.title);
            console.log(res.text);
            console.log(res.date);
            document.querySelector('.promotion-title').innerHTML = res.title;
            document.querySelector('.promotion-text').innerHTML = res.text;
            document.querySelector('.promotion-date').innerHTML = res.date;
        })
        .catch(err => {
            if (err) {
                throw err;
